  ...
  if (nfts.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-[var(--text-secondary)] font-medium">No NFTs found</p>
        <p className="text-sm text-[var(--text-muted)] mt-1">Add a wallet with NFTs to see them here</p>
      </div>
    );
  }
    ...
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 pb-5 border-b border-[var(--border)]">
          <div className="text-sm text-[var(--text-secondary)]">
            ...
            {!showReceived && ownedNfts.length > 0 && (
              <span className="text-[var(--text-muted)] hidden sm:inline ml-1">(minted or purchased)</span>
            )}
          </div>
          <button
            ...
            className={`
              ...
              ${showReceived
                ...
                : 'bg-[var(--bg-tertiary)] text-[var(--text-secondary)] border border-[var(--border)] hover:border-[var(--border-hover)]'
              }
            `}
          >
            <span className={`w-4 h-4 rounded flex items-center justify-center transition-colors ${showReceived ? 'bg-[var(--accent-green)]' : 'border border-[var(--text-muted)]'}`}>
              {showReceived && (
            ...
            Include received ({receivedNfts.length})
          </button>
        </div>
      )}
      ...
      {filteredNfts.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-[var(--text-muted)]">No owned NFTs found</p>
          {receivedNfts.length > 0 && (
            <button
        ...
        <>
          ...
          {hasMore && (
            <button
              ...
              className="mt-5 w-full py-3 text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors border border-[var(--border)] rounded-xl hover:bg-[var(--bg-glass)] hover:border-[var(--border-hover)]"
            >
...
function NFTCard({ nft, index }: { nft: NFT; index: number }) {
  ...
  const getAcquisitionBadge = () => {
    ...
    if (type === 'minted' || (price !== undefined && price > 0 && type !== 'purchased')) {
      return { label: 'Minted', color: 'text-purple-400 bg-purple-500/15 border-purple-500/20' };
    }
    if (type === 'purchased' || (price !== undefined && price > 0)) {
      return { label: 'Bought', color: 'text-blue-400 bg-blue-500/15 border-blue-500/20' };
    }
    if (type === 'received' || price === 0) {
      return { label: 'Received', color: 'text-[var(--text-muted)] bg-[var(--bg-tertiary)] border-[var(--border)]' };
    }
    return null;
  };
  ...
  return (
    <div
      className="group relative bg-[var(--bg-secondary)] rounded-xl overflow-hidden border border-[var(--border)] hover:border-[var(--border-hover)] transition-all duration-200 animate-fadeIn"
      style={{ animationDelay: `${index * 30}ms` }}
    >
      {/* Badges */}
      <div className="absolute top-2.5 left-2.5 right-2.5 z-10 flex items-center justify-between">
        {badge && (
          <span className={`text-[9px] font-semibold uppercase px-2 py-1 rounded-md border ${badge.color}`}>
            {badge.label}
          </span>
        )}
        {nft.purchasePrice !== undefined && nft.purchasePrice > 0 && (
          <span className="text-[10px] font-mono text-[var(--accent-green)] px-2 py-1 rounded-md bg-black/60 backdrop-blur-sm border border-[var(--accent-green)]/20">
      ...
      <div className="aspect-square bg-[var(--bg-tertiary)] overflow-hidden">
        {nft.imageUrl && !imgError ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[var(--bg-tertiary)] to-[var(--bg-hover)]">
            <div className="flex flex-col items-center gap-2">
              <svg className="w-8 h-8 text-[var(--text-muted)] opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                <rect x="3" y="3" width="18" height="18" rx="2" />
                <circle cx="8.5" cy="8.5" r="1.5" />
                <path d="M21 15l-5-5L5 21" />
              </svg>
              <span className="text-[8px] text-[var(--text-muted)] uppercase tracking-wider">No Image</span>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Info */}
      <div className="p-3">
        <p className="text-sm font-medium truncate text-[var(--text-primary)]" title={nft.name}>
          {nft.name}
        </p>
        <div className="flex items-center justify-between gap-1 mt-1">
          ...
          {nft.purchaseDate && (
            <p className="text-[10px] text-[var(--text-muted)] shrink-0 font-mono">
